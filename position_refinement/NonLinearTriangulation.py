...
from .MiscUtils import ProjectionMatrix
...
#     u1,v1 = pts1[:, 0], pts1[:, 1]
#     u1_proj = np.divide(X @ p1_1T.flatten() , X @ p1_3T.flatten())
#     v1_proj = np.divide(X @ p1_2T.flatten() , X @ p1_3T.flatten())
#     E1= np.square(v1 - v1_proj) + np.square(u1 - u1_proj)
#
#
#     ## reprojection error for second camera points - j = 2
#     u2,v2 = pts2[:, 0], pts2[:, 1]
#     u2_proj = np.divide(X @ p2_1T.flatten(), X @ p2_3T.flatten())
#     v2_proj = np.divide(X @ p2_2T.flatten(), X @ p2_3T.flatten())
#     E2= np.square(v2 - v2_proj) + np.square(u2 - u2_proj)
#
#     error = E1 + E2
#     errors = np.repeat(error, 4)
#     return errors
...
def NonLinearTriangulation(K, pts1, pts2, x3D, R1, C1, R2, C2):
    ...
    for i in range(len(x3D)):
        optimized_params = optimize.least_squares(fun=ReprojectionLoss, x0=x3D[i], args=[pts1[i], pts2[i], P1, P2])
...
#         raise 'Check point dimensions - level nlt'
#
#     optimized_params = optimize.least_squares(fun=ReprojectionLoss, x0=x3D.flatten(), method="lm",
#                                               args=[pts1, pts2, P1, P2], verbose=2)
#     X1 = optimized_params.x
#     return X1.reshape(-1, 4)